...
# I defined the userb_path function so I can call it in start_here.
def userb_path(jobs_graph):
...
# I moved the graph building to main, and added the userb_path function to main as well.
if __name__ == '__main__':
    # Build the graph with real data
    # Moved graph building to the main function so that it doesn't run during unit tests and will work with start_here.
    jobs_graph = build_jobs_graph("data/Skills.xlsx") 

    # Allows userb path to run when userb_feature_v2.py is run directly, but not during unit tests.
